...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  return {
    title: params.locale === 'fr' ? 'Conditions Générales d\'Utilisation' : 'Terms of Use',
    description: params.locale === 'fr'
      ? 'Conditions générales d\'utilisation de la plateforme NEO Cyber Camp.'
      : 'NEO Cyber Camp platform Terms of Use.',
  };
}

export default async function CGUPage({ params }: PageProps) {
  const dict = await getDictionary(params.locale);
  const l = dict.legal.cgu;
